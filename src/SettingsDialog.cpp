...
void SettingsDialog::buildUi() {
  ...
  // ----- Privacy -----
  {
    ...
    auto [card, cl] = makeCard(tr("Anonymous usage data"),
    ...
    auto [idCard, ic] = makeCard(tr("Anonymous ID"),
        tr("Resetting the ID makes future events appear as a fresh install. "
           "Useful if you'd like to disassociate this machine from prior data."));
    auto* resetIdBtn = new QPushButton(tr("Reset anonymous ID"));
    resetIdBtn->setObjectName("settingsBrowseBtn");
    resetIdBtn->setCursor(Qt::PointingHandCursor);
    connect(resetIdBtn, &QPushButton::clicked, this, [] {
      TelemetryClient::instance()->resetDeviceId();
    });
    auto* idRow = new QHBoxLayout;
    idRow->addWidget(resetIdBtn);
    idRow->addStretch(1);
    ic->addLayout(idRow);

    auto* dashLink = new QLabel(tr(
        "<a href=\"https://mac-andreas.github.io/dashboard\">"
        "View live aggregate dashboard →</a>"));
    dashLink->setOpenExternalLinks(true);
    dashLink->setTextInteractionFlags(Qt::TextBrowserInteraction);
    ic->addWidget(dashLink);
    v->addWidget(idCard);
    v->addStretch(1);
  }
